...
if [[ "${TARGET}" = "production" ]]; then
	sed "s/@CODENAME@/${RELENG_SUITE}/g" /etc/cutie-build/cutie-production.list > /etc/apt/sources.list.d/cutie-production.list
else
	sed "s/@CODENAME@/${RELENG_SUITE}/g" /etc/cutie-build/cutie-staging.list > /etc/apt/sources.list.d/cutie-staging.list
fi
